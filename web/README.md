...
| ✅ **タクティクス・コア**（スカウティング＋必勝プラン＋音声プレビュー） | はい |
| ✅ **3D 戦術ボード**（Three.js、ドローン↔POV、デンジャーゾーン） | はい |
| ✅ **個人レッスン（AI 骨格診断）** | はい（MediaPipe Tasks Vision Web） |
| ✅ **ライブカメラ撮影 → 解析** | はい |
| ✅ **動画ファイルアップロード → 解析** | はい（mp4/mov/webm 等、ブラウザ内処理） |
| ✅ **関節角度の自動測定**（肘・膝・捻転） | はい |
...
#### 動画ファイルアップロードモード
1. ファイル選択 → `URL.createObjectURL` でローカル URL 生成
2. 隠し `<video>` に読み込み、再生速度 2x で再生
3. `requestAnimationFrame` で `video.currentTime` を見ながら 1 フレームずつ検出
4. プログレスバーで進捗表示、骨格オーバーレイで動作確認
5. 再生完了後に `SwingAnalyzer` で解析
6. **動画はサーバーに一切送信されません**（IndexedDB すら保存せず、解析結果だけ保存）
5. 各スイングのインパクトフレームから：
   - 肘・膝の関節角度
   - 肩・骨盤の傾き → **捻転差**
